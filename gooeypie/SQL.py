...
def encode(userID, password, randpassword): #Inserts the UserID encoded password and decoded password into the encdec table
...
def passcheck(encpassword): #Checks to find if the encoded password is in the encdec table
    connect = sqlite3.connect(DBpath)
    cursor = connect.cursor() 
...
def randpassdisplay(UserID): #Retrieves the encoded password assigned with same UserID
    connect = sqlite3.connect(DBpath)
    cursor = connect.cursor() 
...
def retrieve_userID(password1 , password2, password3): #Retrieves the Primary key from the desired line and replaces the primary key in the encdec table
    connect = sqlite3.connect(DBpath)
    cursor = connect.cursor()
...
def decode(userID, encpassword): #This function decodes the encoded password through comparing the userID and encoded password, with the data stored in the encdec table 
    connect = sqlite3.connect(DBpath)
    cursor = connect.cursor() 
